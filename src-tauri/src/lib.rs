...
use std::sync::Mutex;
use suite_commands::{
    precache_add, precache_list, precache_proxies_folder, precache_remove, set_drive_providers,
};
...
impl AppState {
    /// Set each drive's provider (Local / Suite / LucidLink) from the frontend map.
    pub(crate) fn set_drive_providers(&self, providers: HashMap<String, String>) {
    ...
    /// The provider for the drive containing `path`.
    pub(crate) fn provider_for(&self, path: &Path) -> cache::Provider {
        let pool = self.0.lock().unwrap();
        drive_root_of(path)
            .and_then(|root| pool.drive_providers.get(&norm_path(&root)).copied())
            .unwrap_or(cache::Provider::Local)
    }

    /// Snapshot of the drive→provider map (for listing pinned paths).
    pub(crate) fn drive_providers_snapshot(&self) -> HashMap<String, cache::Provider> {
        self.0.lock().unwrap().drive_providers.clone()
    }
